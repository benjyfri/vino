...
def run_and_check(config_path, expected_mode, tmp_path):
...
def test_bbbp_debug_tiny(tmp_path):
    run_and_check("configs/experiments/bbbp_debug_tiny.yaml", "resize_bilinear", tmp_path)

def test_bbbp_debug_tiny_seed(tmp_path):
    out_dir = str(tmp_path)
    config_path = "configs/experiments/bbbp_debug_tiny.yaml"
    result = subprocess.run([
        "python", "scripts/train.py", "--config", config_path, "--output_dir", out_dir, "--seed", "123"
    ], capture_output=True, text=True)
...
def test_bbbp_debug_tiny_resize(tmp_path):
    run_and_check("configs/experiments/bbbp_debug_tiny_resize.yaml", "resize_bilinear", tmp_path)

def test_bbbp_debug_tiny_patchalign(tmp_path):
    run_and_check("configs/experiments/bbbp_debug_tiny_patchalign.yaml", "patch_aligned_repeat", tmp_path)
